...
if (isset($_GET['s']) && $_GET['s'] != "") {
  $query1 = mysqli_query($conn, "select * from tblslide where idslide = " . $_GET['s']);
...
<body onload="initialize();">
  ...
  <div class="row"></div>
  <div class="container">
    <h1 class="text-center">Edit Slide Show</h1>
    <form action="p_editslide.php" method="post" enctype="multipart/form-data">
      <input type="hidden" name="ids" value="<?php echo $data1['idslide']; ?>" />
      <div class="input-group" style="margin-bottom:10px">
        <span class="input-group-addon"><i class="fa fa-image"></i> Slide Show Image</span>
        <img id="imgpreview" src="../public/images/banner/<?php echo $data1['gambar']; ?>" width='200' height="200"
      ...
      <div class="input-group" style="margin-bottom:10px">
        ...
        <select name="brand" class="form-control">
          ...
          while ($data = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
            ?>
            <option <?php if ($data1['idbrand'] == $data['idbrand']) {
              echo "selected";
            } ?>
      ...
      <input type="submit" class="btn btn-primary" value="Edit Slide Show" />
    </form>
  </div>
